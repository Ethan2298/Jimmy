...
```
Dealer (browser) <-> Next.js App Router <-> Codex API (Marcus) <-> GHL API
                          |
                      Supabase
                   (auth, chat history)
```
...
1. **`/chat`** — Marcus, the dealer-facing AI co-worker. Uses `Codex-sonnet-4.5` with GHL tools. Auth via Supabase. Messages persisted to `chat_sessions`/`messages` tables.
...
- **`src/app/oauth/`, `src/app/api/oauth/`** — RFC 7591 dynamic client registration for external MCP connectors (e.g., Codex.ai).
...
Separate Electron desktop app (electron-vite, React 19, Tailwind v4). Has its own `package.json`, `AGENTS.md`, and build system. Excluded from the root TypeScript config.
...
- `ANTHROPIC_API_KEY` — for Marcus (Codex API)
