...
async def get_location_custom_fields() -> str:
    ...
    """
    try:
        data = await get_client().get(f"/locations/{get_client().location_id}/customFields", inject_location=False)
...
async def get_contact_notes(contact_id: str) -> str:
    ...
    """
    try:
        data = await get_client().get(f"/contacts/{contact_id}/notes", inject_location=False)
