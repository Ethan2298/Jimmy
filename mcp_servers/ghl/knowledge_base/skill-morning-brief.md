...
## Instructions
1. Call `pipelines` to get all pipelines and their stages.
2. Call `opportunities` with `action: "search"`, `status: "open"`, and `limit: 50` to get active deals.
...
4. Call `conversations` with `action: "search"` and `limit: 10` to get recent inbox activity.
5. Call `calendars` with `action: "list"`, then `calendars` with `action: "events"` for today's date range (start of day to end of day, use America/New_York timezone).
