...
export function isComposerSendDisabled(input: string, isStreaming: boolean, disabled: boolean): boolean {
  return disabled || !input.trim() || isStreaming;
}
...
export function ComposerDock({
  ...
  const sendDisabled = isComposerSendDisabled(input, isStreaming, disabled);
      ...
      <div className="w-full max-w-[704px] rounded-[16px] border border-white/[0.08] bg-white/[0.03]">
        <div className="px-3.5 pt-3 pb-2">
          <textarea
            ...
            disabled={disabled}
            placeholder={disabled ? disabledHint : "Message..."}
            rows={1}
            className={cn(
        ...
        <div className="h-[48px] px-2.5 pb-2 flex items-center">
          ...
          {isStreaming ? (
            <button
              onClick={onCancel}
              className="magi-no-drag w-8 h-8 rounded-full flex items-center justify-center bg-white/[0.12] text-white/70 hover:bg-white/[0.18] transition-colors"
              title="Stop (Esc)"
            >
            ...
            <button
              ...
              className={cn(
                "magi-no-drag w-8 h-8 rounded-full flex items-center justify-center transition-opacity",
                !sendDisabled
